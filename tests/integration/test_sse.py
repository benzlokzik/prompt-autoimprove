...
async def test_sse_stream_emits_pipeline_stages(client: httpx.AsyncClient) -> None:
    ...
    body = {"prompt": "Summarize this article", "profile": "qwen3-7b"}
    seen_events: list[str] = []
    async with client.stream("POST", "/v1/improve/stream", headers=headers, json=body) as resp:
...
@pytest.mark.asyncio
async def test_sse_stream_handles_long_prompt(client: httpx.AsyncClient) -> None:
    # A long prompt would overflow a GET query string; POST carries it in the body.
    headers = {"x-api-key": "test-key", "accept": "text/event-stream"}
    body = {"prompt": "Refactor this service. " * 500, "profile": "qwen3-7b"}
    seen_events: list[str] = []
    async with client.stream("POST", "/v1/improve/stream", headers=headers, json=body) as resp:
        assert resp.status_code == 200
        async for line in resp.aiter_lines():
            if line.startswith("event:"):
                ...
                if "final_decision" in seen_events:
                    break
    assert "final_decision" in seen_events
